...
namespace DynamicData.Admin.API
{
    ...
    {
        [HttpPost()]
        [ActionName("UploadSummernotePhoto")]
        public string UploadSummernotePhoto()
        {
            HttpPostedFile file = HttpContext.Current.Request.Files["SummernotePhoto"];
            string frontendPhysicalPath = Utility.FrontendPhysicalPath;
            string frontendVirtualPath = Utility.FrontendVirtualPath;
            string summernotePhotoDirectoryPath = WebConfigurationManager.AppSettings["SummernotePhotoDirectoryPath"];
            string targetFolder = frontendPhysicalPath + summernotePhotoDirectoryPath;
            if (Directory.Exists(targetFolder) == false)
                Directory.CreateDirectory(targetFolder);
            string fileUrl = string.Format("{0}{1}_{2}", targetFolder, Utility.GetRand(), file.FileName);
            file.SaveAs(fileUrl);
            return fileUrl.Replace(frontendPhysicalPath, frontendVirtualPath);
        }
